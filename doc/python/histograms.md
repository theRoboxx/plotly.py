...
You can add text to histogram bars with the `texttemplate` argument. In this example we add the x-axis values as  text using the format `%{variable}`. We also adjust the size of the text using `textfont_size`.
